...
def format_polling_notification(
    ...
    """Format a polling-based notification with before/after context."""
    title_map = {
    ...
    if before_data and after_data and change_type == "update":
        ...
        for column in changed_columns:
            ...
            lines.append(
                f"• <b>{_escape_telegram_html(column)}</b>: {_escape_telegram_html(before_value)} → {_escape_telegram_html(after_value)}"
            )
